...
func (r *Relay) publishPending(ctx context.Context) error {
	...
	for _, row := range rows {
		// "auth-" prefix keeps Msg-Id dedup keys from colliding with the
		// subscription and monitoring relays, which use bare / "mon-" ids.
		msgID := "auth-" + strconv.FormatInt(row.ID, 10)
