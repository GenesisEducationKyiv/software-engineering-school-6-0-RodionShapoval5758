...
func (uc *Subscribe) createPending(ctx context.Context, email, repoName string, repositoryID int64) error {
	for range maxTokenAttempts {
		...
		err = uc.createAndEnqueue(ctx, *sub, payload)
...
func (uc *Subscribe) createAndEnqueue(ctx context.Context, sub domain.Subscription, payload []byte) error {
	...
	if err := uc.outbox.Insert(ctx, tx, contract.SubjectConfirmation, payload); err != nil {
